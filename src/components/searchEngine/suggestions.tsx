...
import {Dispatch, KeyboardEventHandler, SetStateAction, useCallback, useEffect, useRef, useState} from "react";
...
export default function Suggections(props: SuggestProps) {
    ...
    return <SuggestContainer ref={suggestRef} onKeyDown={chooseSuggestItem}>
        {query.length >= 0 ?
            loading ?'loading' : suggest.map((item:string,index:number) => <SuggestItem key={index}>{item}</SuggestItem>) : null
        }
    </SuggestContainer>
}
...
async function getSuggest(term:string): Promise<any[]> {
    ...
    const suggest =  Object.keys(dictonary).filter(key => key.toLowerCase().includes(term.toLowerCase()))
